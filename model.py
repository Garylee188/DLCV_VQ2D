...
class Bbox_Predictor(nn.Module):
    ...
    def __init__(self):
        ...
        self.match_block = match_block(inplanes=128)
        ...
        # Fully connected layers for processing keypoints
        self.fc_kpts = nn.Sequential(
            nn.Linear(128, 256),
            nn.ReLU()
        )
    ...
    def forward(self, crop, frame, kpts):
        frame_feat = self.maxpool(self.conv1(frame))  # (64, 112, 112)
        print("image_features grad:",frame_feat.grad_fn)
        frame_feat = self.maxpool(self.conv2(frame_feat))  # (128, 56, 56)

        crop_feat = self.maxpool(self.conv1(crop))  # (64, 112, 112)
        crop_feat = self.maxpool(self.conv2(crop_feat))  # (128, 56, 56)

        non_det, act_det, act_aim, c_weight = self.match_block(frame_feat, crop_feat)
        
        act_det = act_det.view(act_det.size(0), -1)
        act_det = self.fc_1(act_det)
        act_det = self.fc_2(act_det)  # (B, 512)

        act_aim = act_aim.view(act_aim.size(0), -1)
        act_aim = self.fc_1(act_aim)
        act_aim = self.fc_2(act_aim)  # (B, 512)

        kpts_feat = self.fc_kpts(kpts)
        kpts_feat = kpts_feat.view(kpts_feat.size(0), -1)
